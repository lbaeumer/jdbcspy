...
import de.luisoft.db.proxy.StatementFactory;
 ...
 * <p>Company: </p>
 * @author Lui Baeumer
 * @version $Id: ConnectionHandler.java 885 2007-03-18 20:46:41Z lui $
 */
public class ConnectionHandler implements InvocationHandler,
    ...
    private List mStatements = new LinkedList();

    /** result set item count */
    private List mResultSetItemCount = new ArrayList();
    ...
    private List mConnectionListener;
     ...
     */
    public ConnectionHandler(ClientProperties props, Connection theConn,
        List listener, List failedListener,
    ...
    public Object invoke(Object proxy, Method method, Object[] args)
        ...
        try {
            ...
            mTrace.error(getMethodSignature(method, args) + " failed for "
                         + toString(), e.getCause());
            throw e.getCause();
        }
    ...
    public String toString() {
        synchronized (mStatements) {
            return "Connection[#stmt=" + getItemCount()
                + "; #rs=" + printList(mResultSetItemCount)
                + "; duration="
                + Utils.getTimeString(getDuration())
                + (getSize() > 0 ? "; size=" + Utils.getSizeString(getSize())
                   : "")
                + ", opened in " + getCaller()
                + "]";
        }
     ...
     */
    private Object handlePrepare(Object proxy, Method method, Object[] args)
        throws Throwable {

        String sql = (mProps.getBoolean(ClientProperties.DB_REMOVE_HINTS)
                      ? Utils.removeHints(args[0].toString())
                      : args[0].toString());

        Object ob = method.invoke(mConn, args);
        if (ob instanceof Statement) {
            Statement proxyStmt = StatementFactory.getInstance().getStatement(
                mProps,
                (Statement) ob,
                mMetaData,
                sql,
     ...
     * @return Object
     */
    private Object handleCreate(Object proxy, Method method, Object[] args)
        throws Throwable {
        Object ob = method.invoke(mConn, args);
        if (ob instanceof Statement) {
            Statement proxyStmt = StatementFactory.getInstance().getStatement(
                mProps,
                (Statement) ob, mMetaData, null, mListener,
                mFailedListener,
                Utils.getExecClass(proxy));
    ...
    private Object handleClose(Object proxy, Method method, Object[] args)
        ...
        if (!mProps.getBoolean(
            ...
            && mIsClosed) {
            String txt = "The connection " + this + " was already closed in "
                         + Utils.getExecClass(proxy) + ".";
        ...
        try {
            ...
            for (Iterator it = mConnectionListener.iterator();
            	it.hasNext(); ) {
        ...
        boolean displayTime = mDuration > mProps.getInt(
            ClientProperties.DB_CONN_TOTAL_TIME_THRESHOLD);
        boolean displaySize = mDuration > mProps.getInt(
            ClientProperties.DB_CONN_TOTAL_SIZE_THRESHOLD);

        if (displayTime || displaySize) {
            mTrace.info("closed " + this
                        + " in " + Utils.getExecClass(proxy));
        }
    ...
    /**
     * @see de.luisoft.db.proxy.ConnectionStatistics#getProxyConnectionMetaData
     */
