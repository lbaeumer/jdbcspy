...
public class ExecutionStatisticListener extends ExecutionAdapter {
	...
	public String toString() {
		...
		if (mStmtCount != 0) {
			map.append("; avgDuration=" + utils.getTimeString(mDuration / mStmtCount) + "; maxLength=" + mMaxLength
					+ "; avgLength=" + (mTotalLength / mStmtCount) + ";\ntime=");

			boolean first = true;
			synchronized (mTimeMap) {
				for (int i = 0; i < TIME_CAT_STRING.length; i++) {
					Integer count = mTimeMap.get(TIME_CAT_STRING[i]);
					if (count != null) {
						if (!first) {
							map.append(", ");
						}
			...
			synchronized (mLengthMap) {
				for (int i = 0; i < LENGTH_CAT_STRING.length; i++) {
					Integer count = mLengthMap.get(LENGTH_CAT_STRING[i]);
					if (count != null) {
						if (!first) {
							map.append(", ");
						}
